...
def test_example(page: Page) -> None:
    ...
    page.get_by_role("combobox").filter(has_text="Config Packages").click()
    page.get_by_role("option", name="Description").click()
    page.get_by_role("combobox").filter(has_text=re.compile(r"^$")).click()
    page.get_by_role("option", name="Config Packages").click()
    page.get_by_role("combobox").filter(has_text=re.compile(r"^$")).click()
    page.get_by_role("option", name="Release Name").click()
    page.get_by_role("combobox").filter(has_text=re.compile(r"^$")).click()
    page.get_by_role("option", name="Creation Date").click()
    page.get_by_role("combobox").filter(has_text=re.compile(r"^$")).click()
